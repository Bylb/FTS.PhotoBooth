namespace FTS.PhotoBooth.Video
{
    #region Namespace
    ...
    public partial class WebcamDevice
    {
        ...
        /// Dependency property for take snapshot command.
        /// </summary>
        public static readonly DependencyProperty TimeBeforeCaptureProperty = DependencyProperty.RegisterAttached("TimeBeforeCapture", typeof(int), typeof(WebcamDevice), new PropertyMetadata(TimeBeforeCapturePropertyChangedCallback));


        /// <summary>
        /// Dependency property for take snapshot command.
        /// </summary>
        public static readonly DependencyProperty InitialTimeBeforeCaptureProperty = DependencyProperty.RegisterAttached("InitialTimeBeforeCapture", typeof(int), typeof(WebcamDevice), new PropertyMetadata(InitialTimeBeforeCapturePropertyChangedCallback));
        ...
        public WebcamDevice()
        {
            ...
            currentDispatch = this.Dispatcher;
        ...
        private int timeBeforeCapture; 
        /// <summary>
        /// Gets or sets take snapshot command.
        /// </summary>
        public int TimeBeforeCapture
        {
            get
            {
                return timeBeforeCapture; 
//                return (int)GetValue(TimeBeforeCaptureProperty);
            }

            set
            {
                timeBeforeCapture = value; 
                 currentDispatch.BeginInvoke(DispatcherPriority.Background, new Action(() =>  this.SetValue(TimeBeforeCaptureProperty, value)));
            }
        }
        ...
        /// Gets or sets take snapshot command.
        /// </summary>
        public int InitialTimeBeforeCapture
        {
            get
            {
                return (int)GetValue(InitialTimeBeforeCaptureProperty);
            }

            set
            {
                this.SetValue(InitialTimeBeforeCaptureProperty, value);
            }
        }
        ...
        {
          TimeBeforeCapture--  ; 
           if (TimeBeforeCapture <= 0)
            {
                //Stop the Timer
                captureTimer.Stop();
                currentDispatch.BeginInvoke(DispatcherPriority.Background, new Action(() =>
                    {
                        try
                        {
                            this.SnapshotBitmap = pic; 
        ...
        {
            if (InitialTimeBeforeCapture == null)
                InitialTimeBeforeCapture = 5;

            TimeBeforeCapture = InitialTimeBeforeCapture;           
            captureTimer.Start(); 
        }
        ...
        private static void TimeBeforeCapturePropertyChangedCallback(DependencyObject sender, DependencyPropertyChangedEventArgs eventArgs)
        {
            //// NOTE: Created to make the dependency property bindable from view-model.
        }


        private static void InitialTimeBeforeCapturePropertyChangedCallback(DependencyObject sender, DependencyPropertyChangedEventArgs eventArgs)
        {
            //// NOTE: Created to make the dependency property bindable from view-model.
        }
        ...
        private void InitializeVideoDevice(string videoDeviceSourceId)
        {
            ...
            else
            {
                try
                {
                    ...
                    this.videoCaptureDevice.VideoResolution = test.Last();
        ...
        {
            pic = (Bitmap)eventArgs.Frame.Clone(); 
        }
